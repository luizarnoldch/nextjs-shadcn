...
export default function InputGroupDemoPage() {
  const [align, setAlign] = useState<"inline-start" | "inline-end" | "block-start" | "block-end">("inline-start");
  const [addonType, setAddonType] = useState<"text" | "icon" | "button">("text");
  ...
  const generateCode = () => {
    ...
    if (inputType === "input") {
      parts.push(`      <InputGroupInput placeholder="example.com"${invalidAttr} />`);
    } else {
      parts.push(`      <InputGroupTextarea placeholder="Type your message..."${invalidAttr} />`);
    }
    ...
    const extraImports = addonType === "icon" ? `\nimport { Search } from "lucide-react"` : "";
...
${innerJSX}
  ...
  const highlightedCode = generateCode()
    ...
    // Tags
    .replace(/&lt;(\/?)InputGroup([A-Za-z]*)(.*?)&gt;/g, `&lt;$1<span class="${hlTag}">InputGroup$2</span>$3&gt;`)
    .replace(/&lt;(\/?)Search(.*?)&gt;/g, `&lt;$1<span class="${hlTag}">Search</span>$2&gt;`)
    // Attributes
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid gap-1">
        ...
        <p className="text-muted-foreground">
          A stylized wrapper combining inputs, text addons, and buttons into a seamless line.
        </p>
      </div>
      ...
      <div className="flex flex-col xl:flex-row gap-8 mt-6">
        ...
        <div className="flex flex-col gap-6 w-full xl:w-100 p-6 border rounded-lg bg-card shadow-sm h-fit">
          ...
          <div className="flex flex-col gap-4">
            ...
            <h3 className="text-md font-medium text-muted-foreground">Addon Settings</h3>
            ...
            <h3 className="text-md font-medium text-muted-foreground">Input Settings</h3>

            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-sm font-medium">Control Type</span>
                <span className="text-xs text-muted-foreground">Changes the inner field</span>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Set <code>aria-invalid</code></span>
              </div>
              <Button
          ...
          <div className="relative z-10 w-full flex justify-center p-8 bg-card border rounded-xl shadow-sm min-w-75 overflow-hidden">
            ...
            <InputGroup className="max-w-md w-full transition-all">
              <InputGroupAddon align={align}>
                {addonType === "text" && <InputGroupText>https://</InputGroupText>}
                {addonType === "icon" && (
                  <InputGroupText>
                    <Search className="size-4" />
                  </InputGroupText>
                )}
                {addonType === "button" && <InputGroupButton>Paste</InputGroupButton>}
              </InputGroupAddon>

              {inputType === "input" ? (
                <InputGroupInput placeholder="example.com" aria-invalid={invalid} />
              ) : (
                <InputGroupTextarea placeholder="Type your message..." aria-invalid={invalid} />
              )}
            </InputGroup>
