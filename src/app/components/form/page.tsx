...
export default function FormDemoPage() {
  ...
  const formSchema = useMemo(() => {
    return z.object({
      username: requireUsername
        ...
        : z.string().optional(),
      email: requireEmail
        ? z.string().email({ message: "Invalid email address." }).min(1, { message: "Email is required." })
        : z.string().optional(),
    });
  ...
  const generateCode = () => {
    ...
    parts.push(`              <FormControl>`);
    parts.push(`                <Input type="email" placeholder="m@example.com" {...field} />`);
    parts.push(`              </FormControl>`);
    if (showFormDescriptions) {
      parts.push(`              <FormDescription>`);
...
${innerJSX}
  ...
  const highlightedCode = generateCode()
    ...
    // Tags
    .replace(/&lt;(\/?)Form([A-Za-z]*)(.*?)&gt;/g, `&lt;$1<span class="${hlTag}">Form$2</span>$3&gt;`)
    .replace(/&lt;(\/?)form(.*?)&gt;/g, `&lt;$1<span class="${hlTag}">form</span>$2&gt;`)
    ...
    .replace(/ name="([^"]+)"/g, ` <span class="${hlProp}">name</span>="<span class="${hlVal}">$1</span>"`)
    ...
    .replace(/ onSubmit=\{([^}]+)\}/g, ` <span class="${hlProp}">onSubmit</span>={<span class="${hlVal}">$1</span>}`)
    .replace(/ control=\{([^}]+)\}/g, ` <span class="${hlProp}">control</span>={<span class="${hlVal}">$1</span>}`)
    .replace(/ render=\{([^}]+)\}/g, ` <span class="${hlProp}">render</span>={<span class="${hlVal}">$1</span>}`);
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid gap-1">
        ...
        <p className="text-muted-foreground">
          Building forms with React Hook Form and Zod. Accessible, schema-driven, and highly composable.
        </p>
      </div>
      ...
      <div className="flex flex-col xl:flex-row gap-8 mt-6">
        ...
        <div className="flex flex-col gap-6 w-full xl:w-[400px] p-6 border rounded-lg bg-card shadow-sm h-fit">
          ...
          <div className="flex flex-col gap-4">
            ...
            <h3 className="text-md font-medium text-muted-foreground">Zod Schema Validation</h3>

            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-sm font-medium">Require Username</span>
                <span className="text-xs text-muted-foreground"><code>.string().min(2)</code></span>
              </div>
              <Button
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground"><code>.string().email()</code></span>
              </div>
              <Button
            ...
            <h3 className="text-md font-medium text-muted-foreground">Form Structure</h3>

            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-sm font-medium">Descriptions</span>
                <span className="text-xs text-muted-foreground">Toggles <code>&lt;FormDescription&gt;</code></span>
              </div>
              <Button
