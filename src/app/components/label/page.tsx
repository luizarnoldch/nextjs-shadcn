...
export default function LabelExample() {
  ...
  const highlightedCode = rawCodeContent
    ...
    .replace(/ htmlFor=\"([^\"]+)\"/g, ' <span class="text-purple-400">htmlFor</span>="<span class="text-blue-400">$1</span>"')
    .replace(/ data-disabled=\"true\"/g, ' <span class="text-purple-400">data-disabled</span>="<span class="text-blue-400">true</span>"');
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid gap-1">
        ...
        <p className="text-muted-foreground">
          Componente para mostrar etiquetas asociadas a campos de formulario. Prueba las variaciones y copia el código generado.
        </p>
      </div>
      ...
      <div className="flex flex-col xl:flex-row gap-8 mt-6">
        ...
        <div className="flex flex-col gap-6 w-full xl:w-100 p-6 border rounded-lg bg-card shadow-sm h-fit">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant={withHtmlFor ? "default" : "secondary"} size="sm" onClick={() => setWithHtmlFor(!withHtmlFor)}>{withHtmlFor ? "Activado" : "Desactivado"}</Button>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Disabled</span>
              <Button variant={disabled ? "default" : "secondary"} size="sm" onClick={() => setDisabled(!disabled)}>{disabled ? "Sí" : "No"}</Button>
            </div>
            <div className="flex items-center justify-between">
        ...
        <div className="flex-1 p-10 border rounded-lg bg-zinc-100 dark:bg-zinc-950 flex flex-col items-center justify-center relative min-h-30">
          ...
          <div className="relative z-10 w-full flex justify-center p-8 bg-card border rounded-xl shadow-sm min-w-30 overflow-hidden">
            <Label htmlFor={withHtmlFor ? "input-id" : undefined} data-disabled={disabled ? true : undefined}>{labelText}</Label>
          </div>
        </div>
      </div>
