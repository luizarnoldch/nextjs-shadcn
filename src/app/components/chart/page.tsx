...
${innerJSX}
  ...
  const highlightedCode = generateCode()
    ...
    .replace(
      /&lt;(\/?)Chart([A-Za-z]*)(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">Chart$2</span>$3&gt;`,
    )
    .replace(
      /&lt;(\/?)Bar([A-Za-z]*)(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">Bar$2</span>$3&gt;`,
    )
    .replace(
      /&lt;(\/?)Line([A-Za-z]*)(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">Line$2</span>$3&gt;`,
    )
    .replace(
      /&lt;(\/?)Area([A-Za-z]*)(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">Area$2</span>$3&gt;`,
    )
    .replace(
      /&lt;(\/?)Pie([A-Za-z]*)(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">Pie$2</span>$3&gt;`,
    )
    .replace(
      /&lt;(\/?)CartesianGrid(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">CartesianGrid</span>$2&gt;`,
    )
    .replace(
      /&lt;(\/?)XAxis(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">XAxis</span>$2&gt;`,
    )
    // Attributes
    .replace(
      / className="([^"]+)"/g,
    ...
    .replace(
      / dataKey="([^"]+)"/g,
      ` <span class="${hlProp}">dataKey</span>="<span class="${hlVal}">$1</span>"`,
    )
    .replace(
      / nameKey="([^"]+)"/g,
      ` <span class="${hlProp}">nameKey</span>="<span class="${hlVal}">$1</span>"`,
    )
    .replace(
      / fill="([^"]+)"/g,
      ` <span class="${hlProp}">fill</span>="<span class="${hlVal}">$1</span>"`,
    )
    .replace(
      / stroke="([^"]+)"/g,
      ` <span class="${hlProp}">stroke</span>="<span class="${hlVal}">$1</span>"`,
    )
    .replace(
      / type="([^"]+)"/g,
    ...
    .replace(
      / config=\{([^}]+)\}/g,
      ` <span class="${hlProp}">config</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / data=\{([^}]+)\}/g,
      ` <span class="${hlProp}">data</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / content=\{([^}]+)\}/g,
      ` <span class="${hlProp}">content</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / vertical=\{([^}]+)\}/g,
      ` <span class="${hlProp}">vertical</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / tickLine=\{([^}]+)\}/g,
      ` <span class="${hlProp}">tickLine</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / tickMargin=\{([^}]+)\}/g,
      ` <span class="${hlProp}">tickMargin</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / axisLine=\{([^}]+)\}/g,
      ` <span class="${hlProp}">axisLine</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / strokeWidth=\{([^}]+)\}/g,
      ` <span class="${hlProp}">strokeWidth</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / dot=\{([^}]+)\}/g,
      ` <span class="${hlProp}">dot</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / radius=\{([^}]+)\}/g,
      ` <span class="${hlProp}">radius</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / innerRadius=\{([^}]+)\}/g,
      ` <span class="${hlProp}">innerRadius</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / fillOpacity=\{([^}]+)\}/g,
      ` <span class="${hlProp}">fillOpacity</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / cursor=\{([^}]+)\}/g,
      ` <span class="${hlProp}">cursor</span>={<span class="${hlVal}">$1</span>}`,
    )
    .replace(
      / tickFormatter=\{([^}]+)\}/g,
      ` <span class="${hlProp}">tickFormatter</span>={<span class="${hlVal}">$1</span>}`,
    );
          ...
          <div className="relative z-10 w-full rounded-xl border bg-card p-6 shadow-sm">
            <div className="mb-6">
              ...
              <p className="text-sm text-muted-foreground">
                {chartType === "pie"
                  ? "Showing browser market share for January"
