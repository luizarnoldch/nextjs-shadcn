...
export default function ItemDemoPage() {
  // Variaciones
  const [variant, setVariant] = useState<"default" | "outline" | "muted">(
    "default",
  );
  const [size, setSize] = useState<"default" | "sm">("default");
  const [mediaType, setMediaType] = useState<"default" | "icon" | "image">(
    "default",
  );
...
${item}
  ...
  const highlightedCode = rawCodeContent
    ...
    .replace(
      /&lt;(\/?)Item(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">Item</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemMedia(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemMedia</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemContent(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemContent</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemActions(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemActions</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemTitle(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemTitle</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemDescription(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemDescription</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemHeader(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemHeader</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemFooter(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemFooter</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemGroup(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemGroup</span>$2&gt;',
    )
    .replace(
      /&lt;(\/?)ItemSeparator(.*?)&gt;/g,
      '&lt;$1<span class="text-emerald-400">ItemSeparator</span>$2&gt;',
    )
    .replace(
      / variant=\"([^\"]+)\"/g,
      ' <span class="text-purple-400">variant</span>="<span class="text-blue-400">$1</span>"',
    )
    .replace(
      / size=\"([^\"]+)\"/g,
      ' <span class="text-purple-400">size</span>="<span class="text-blue-400">$1</span>"',
    );
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid gap-1">
        ...
        <p className="text-muted-foreground">
          Componente para mostrar elementos tipo lista, tarjetas o bloques.
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              {["default", "outline", "muted"].map((v) => (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setVariant(v as "default" | "outline" | "muted")
                  }
                >
                  {v}
                </Button>
            ...
            <div className="flex gap-2 mt-2">
              {["default", "sm"].map((s) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setSize(s as "default" | "sm")}
                >
                  {s}
                </Button>
              ))}
            ...
            <div className="flex gap-2">
              {["default", "icon", "image"].map((m) => (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setMediaType(m as "default" | "icon" | "image")
                  }
                >
                  {m}
                </Button>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                >
                  {showHeader ? "Activado" : "Desactivado"}
                </Button>
              </div>
              <div className="flex items-center justify-between">
                ...
                >
                  {showFooter ? "Activado" : "Desactivado"}
                </Button>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Actions</span>
                <Button
                  variant={showActions ? "default" : "secondary"}
                  size="sm"
                  onClick={() => setShowActions(!showActions)}
                >
                  {showActions ? "Activado" : "Desactivado"}
                </Button>
              </div>
              <div className="flex items-center justify-between">
                ...
                >
                  {showDescription ? "Activado" : "Desactivado"}
                </Button>
              </div>
              <div className="flex items-center justify-between">
