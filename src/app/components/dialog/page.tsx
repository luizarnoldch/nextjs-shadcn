...
export default function DialogDemoPage() {
  ...
  const [preventOutsideInteraction, setPreventOutsideInteraction] = useState(false);
  ...
  const generateCode = () => {
    ...
    if (preventOutsideInteraction) contentProps.push(`onInteractOutside={(e) => e.preventDefault()}`);
    const contentPropsStr = contentProps.length > 0 ? ` ${contentProps.join(" ")}` : "";

    parts.push(`      <DialogContent className="sm:max-w-[425px]"${contentPropsStr}>`);
    parts.push(`        <DialogHeader>`);
    parts.push(`          <DialogTitle>Edit profile</DialogTitle>`);
    parts.push(`          <DialogDescription>`);
    parts.push(`            Make changes to your profile here. Click save when you're done.`);
    parts.push(`          </DialogDescription>`);
    parts.push(`        </DialogHeader>`);
    parts.push(`        <div className="grid gap-4 py-4">`);
    parts.push(`          <div className="grid grid-cols-4 items-center gap-4">`);
    parts.push(`            <Label htmlFor="name" className="text-right">`);
    parts.push(`              Name`);
    parts.push(`            </Label>`);
    parts.push(`            <Input id="name" defaultValue="Pedro Duarte" className="col-span-3" />`);
    parts.push(`          </div>`);
    parts.push(`          <div className="grid grid-cols-4 items-center gap-4">`);
    parts.push(`            <Label htmlFor="username" className="text-right">`);
    parts.push(`              Username`);
    parts.push(`            </Label>`);
    parts.push(`            <Input id="username" defaultValue="@peduarte" className="col-span-3" />`);
    parts.push(`          </div>`);
    parts.push(`        </div>`);
...
${innerJSX}
  ...
  const highlightedCode = generateCode()
    ...
    // Tags
    .replace(/&lt;(\/?)Dialog([A-Za-z]*)(.*?)&gt;/g, `&lt;$1<span class="${hlTag}">Dialog$2</span>$3&gt;`)
    ...
    .replace(/ defaultValue="([^"]+)"/g, ` <span class="${hlProp}">defaultValue</span>="<span class="${hlVal}">$1</span>"`)
    .replace(/ asChild/g, ` <span class="${hlProp}">asChild</span>`)
    .replace(/ showCloseButton(?!=\{)/g, ` <span class="${hlProp}">showCloseButton</span>`)
    // React expressions
    .replace(/ showCloseButton=\{([^}]+)\}/g, ` <span class="${hlProp}">showCloseButton</span>={<span class="${hlVal}">$1</span>}`)
    .replace(/ onInteractOutside=\{([^}]+)\}/g, ` <span class="${hlProp}">onInteractOutside</span>={<span class="${hlVal}">$1</span>}`);
      ...
      <div className="flex flex-col xl:flex-row gap-8 mt-6">
        ...
        <div className="flex flex-col gap-6 w-full xl:w-[400px] p-6 border rounded-lg bg-card shadow-sm h-fit">
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-md font-medium text-muted-foreground">Layout Options</h3>

            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-sm font-medium">Header Close Icon</span>
                <span className="text-xs text-muted-foreground">Toggle the top-right X icon</span>
              </div>
              <Button
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Toggle action close button</span>
              </div>
              <Button
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Prevent click-outside to close</span>
              </div>
              <Button
                variant={preventOutsideInteraction ? "default" : "secondary"}
                onClick={() => setPreventOutsideInteraction(!preventOutsideInteraction)}
                size="sm"
              >
