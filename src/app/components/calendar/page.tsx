...
export default function CalendarPage() {
  ...
  const [dateMultiple, setDateMultiple] = useState<Date[] | undefined>([new Date()]);
  ...
  const generateCodeDynamic = () => {
    ...
    const imports = mode === 'range' ? 'import { addDays } from "date-fns"\\nimport { DateRange } from "react-day-picker"\\n' : '';
...
${imports}export default function CalendarDemo() {
  ...
  const highlightedCode = generateCodeDynamic()
    ...
    .replace(/&lt;\/span&gt;/g, '</span>')
    .replace(/&lt;(\/?)Calendar/g, `&lt;$1<span class="${tag}">Calendar</span>`);
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid gap-1">
        ...
        <p className="text-muted-foreground">
          A date field component that allows users to enter and edit date, multiple dates, or date ranges.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 mt-6">
        ...
        <div className="flex flex-col gap-6 w-full lg:w-[450px] p-6 border rounded-lg bg-card max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-md font-semibold text-blue-400">1. Selection Mode</h3>
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Current State Data: <span className="font-mono text-foreground bg-muted px-1 py-0.5 rounded">{renderSelectionText()}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-md font-semibold text-purple-400">2. Grid Modifiers</h3>

            <div className="flex flex-col gap-2">
              <span className="text-sm font-medium">Outside Days</span>
              <p className="text-xs text-muted-foreground mb-1">Show dates from previous/next months filling empty slots.</p>
              <div className="flex gap-2">
                <Button variant={!showOutsideDays ? "default" : "outline"} size="sm" onClick={() => setShowOutsideDays(false)}>Hide (false)</Button>
                <Button variant={showOutsideDays ? "default" : "outline"} size="sm" onClick={() => setShowOutsideDays(true)}>Show (true)</Button>
              </div>
            </div>

            <div className="flex flex-col gap-2 mt-2">
              <span className="text-sm font-medium">Week Numbers</span>
              <p className="text-xs text-muted-foreground mb-1">Display ISO standard week numbers on the left.</p>
              <div className="flex gap-2">
                <Button variant={!showWeekNumber ? "default" : "outline"} size="sm" onClick={() => setShowWeekNumber(false)}>Disabled</Button>
                <Button variant={showWeekNumber ? "default" : "outline"} size="sm" onClick={() => setShowWeekNumber(true)}>Enabled</Button>
              </div>
            </div>

            <div className="flex flex-col gap-2 mt-2">
              <span className="text-sm font-medium">Fixed Weeks</span>
              <p className="text-xs text-muted-foreground mb-1">Force rendering exactly 6 rows (weeks) regardless of month length to prevent UI jumping.</p>
              <div className="flex gap-2">
                <Button variant={!fixedWeeks ? "default" : "outline"} size="sm" onClick={() => setFixedWeeks(false)}>Disabled</Button>
                <Button variant={fixedWeeks ? "default" : "outline"} size="sm" onClick={() => setFixedWeeks(true)}>Enabled</Button>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-4 text-xs font-mono">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-zinc-400">React State (Generic Types)</span>
              </div>
              <div className="flex items-center gap-2">
