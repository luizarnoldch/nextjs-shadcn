  ...
  const getCodeString = () => {
    ...
    const variantProp =
      variant !== "default"
        ...
        : "";
    const roundedClass = !isRounded
      ? ` className="<span class="${hlLogic}">rounded-md</span>"`
      : "";
        ...
        <>
          ...
          {variant !== "default" ? (
            <>
              ...
              <span className={hlVariant}>{variant}</span>
              <span className={str}>"</span>
            </>
          ) : (
            ""
          ...
          {!isRounded ? (
            <>
              ...
              <span className={str}>"</span>
              <span className={hlLogic}>rounded-md</span>
              <span className={str}>"</span>
            </>
          ) : (
            ""
          ...
          {"        "}
          <span className={hlContent}>{text}</span>
          <br />
          {"      "}
        ...
        {icon !== "none" && (
          <>
            ...
            <span className={tag}>
              &lt;
              <span className={hlContent}>
                {icon === "check" ? "Check" : icon === "flame" ? "Flame" : "X"}
              </span>{" "}
              /&gt;
            </span>
            <br />
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid gap-1">
        ...
        <p className="text-muted-foreground">
          Interactive showcase of the Badge component including variants, custom
          overrides, and polymorphic capabilities.
        </p>
      </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 mt-2">
              ...
              <div className="flex gap-2 flex-wrap">
                <Button
                  variant={icon === "none" ? "default" : "outline"}
                  size="sm"
                ...
                <Button
                  variant={icon === "check" ? "default" : "outline"}
                  size="sm"
                ...
                <Button
                  variant={icon === "flame" ? "default" : "outline"}
                  size="sm"
                  onClick={() => setIcon("flame")}
                >
                  Flame
                </Button>
                <Button
                  variant={icon === "x" ? "default" : "outline"}
                  size="sm"
                  onClick={() => setIcon("x")}
                >
                  X
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 mt-2">
              <span className="text-sm font-medium">
                asChild (Polymorphic Slot)
              </span>
              <p className="text-xs text-muted-foreground leading-tight mb-2">
                Renders the Badge styling onto a different DOM node underneath
                (e.g. an &lt;a&gt; tag for actual active links).
              </p>
              <div className="flex gap-2 flex-wrap">
                <Button
                  variant={!asChild ? "default" : "outline"}
                  size="sm"
                  onClick={() => setAsChild(false)}
                >
                  False (Standard &lt;span&gt;)
                </Button>
                <Button
      ...
      <div className="mt-2 p-6 border rounded-lg bg-zinc-950 text-zinc-50 overflow-x-auto w-full">
        ...
        <pre className="text-sm font-mono whitespace-pre w-full overflow-x-auto p-4 bg-zinc-900/50 rounded-md border border-zinc-800 leading-relaxed">
          <code>
            <span className={kw}>import</span> {"{"} Badge {"}"}{" "}
            <span className={kw}>from</span>{" "}
            <span className={str}>"@/components/ui/badge"</span>
            <br />
            {icon !== "none" && (
              <>
            ...
            <span className="text-blue-400">BadgeDemo</span>() {"{"}
            <br />
            {"  "}
