...
export default function ContextMenuDemoPage() {
  ...
  const generateCode = () => {
    ...
    parts.push(`          Back`);
    if (showShortcuts)
      parts.push(`          <ContextMenuShortcut>⌘[</ContextMenuShortcut>`);
    ...
    parts.push(`          Reload`);
    if (showShortcuts)
      parts.push(`          <ContextMenuShortcut>⌘R</ContextMenuShortcut>`);
    parts.push(`        </ContextMenuItem>`);
    ...
    if (showCheckboxItems) {
      ...
      if (showShortcuts)
        parts.push(`          <ContextMenuShortcut>⌘⇧B</ContextMenuShortcut>`);
...
${innerJSX}
  ...
  const highlightedCode = generateCode()
    ...
    // Tags
    .replace(
      /&lt;(\/?)ContextMenu([A-Za-z]*)(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">ContextMenu$2</span>$3&gt;`,
    )
    // Attributes
    .replace(
      / className="([^"]+)"/g,
