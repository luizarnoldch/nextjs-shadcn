...
export default function InputDemoPage() {
  ...
  const generateCode = () => {
    ...
    if (showLabel) {
      ...
      parts.push(
        `      <Label htmlFor="demo-input">${type.charAt(0).toUpperCase() + type.slice(1)}</Label>`,
      );
      parts.push(
        `      <Input id="demo-input"${typeStr}${placeholderStr}${disabledStr}${invalidStr} />`,
      );
      parts.push(`    </div>`);
    } else {
      parts.push(
        `    <Input className="max-w-sm"${typeStr}${placeholderStr}${disabledStr}${invalidStr} />`,
      );
    }
