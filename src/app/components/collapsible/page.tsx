...
${innerJSX}
  ...
  const highlightedCode = generateCode()
    ...
    .replace(
      /&lt;(\/?)Collapsible(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">Collapsible</span>$2&gt;`,
    )
    .replace(
      /&lt;(\/?)CollapsibleTrigger(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">CollapsibleTrigger</span>$2&gt;`,
    )
    .replace(
      /&lt;(\/?)CollapsibleContent(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">CollapsibleContent</span>$2&gt;`,
    )
    .replace(
      /&lt;(\/?)Button(.*?)&gt;/g,
    ...
    .replace(
      /&lt;(\/?)ChevronsUpDown(.*?)&gt;/g,
      `&lt;$1<span class="${hlTag}">ChevronsUpDown</span>$2&gt;`,
    )
    .replace(
      /&lt;(\/?)div(.*?)&gt;/g,
        ...
        <div className="flex-1 p-10 border rounded-lg bg-zinc-100 dark:bg-zinc-950 flex flex-col items-center justify-center relative min-h-[400px]">
          ...
          <div className="relative z-10 p-12 bg-card border rounded-xl shadow-sm min-w-[350px] flex justify-center">
            ...
            >
              ...
              <div
                className={`rounded-md${borderClass} font-mono text-sm shadow-sm`}
              >
                @radix-ui/primitives
              </div>
